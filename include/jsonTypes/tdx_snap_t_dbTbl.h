...
class tdx_snap_t_dbTbl : public tableIO_t<struct tdx_snap_t>{
public:
    typedef struct tdx_snap_t data_t;
	//tdx_snap_t_dbTbl(const std::string name)
    //{
    //    set_name(name);
    //};
    ...
    virtual void bind_query(SQLite::Statement& query, const std::vector<struct tdx_snap_t>& data, size_t nb) override
    {
        const struct tdx_snap_t & K_data = data[nb];
        query.bind(1, K_data.date);
		query.bind(2, K_data.minuSec);
		query.bind(3, K_data.time);
		...
		query.bind(49, K_data.lcl_time_str);
		query.bind(50, K_data.VERSION);
    }

    //warning: not clear data inside, but append DB.data to it
    virtual void load_query(SQLite::Statement& query, std::vector<tdx_snap_t>& data) override
    {
        struct tdx_snap_t K_data;
        K_data.date = (time_date_t)query.getColumn(0).getInt64();
		K_data.minuSec = (time_minuSec_t)query.getColumn(1).getInt64();
		K_data.time = (time_utcSec_t)query.getColumn(2).getInt64();
